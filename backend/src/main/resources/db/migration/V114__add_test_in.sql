...
CREATE TABLE IF NOT EXISTS `test_case_script_information` (
    ...
    `cover_install` int(11) DEFAULT NULL COMMENT '脚本执行策略:覆盖安装 指脚本执行完是否要覆盖安装1：要覆盖 0：不覆盖',
    ...
    `result_category` int(11) DEFAULT NULL COMMENT '错误结果分类 1 通过,2 安装失败,3 卸载失败,4 启动失败,5 运行失败,6 功能异常,7 未执行,8 超时,9 取消,10 monkey失败,11 警告,12 应用崩溃,13 脚本错误,14 应用无响应,15 环境异常,16 遍历失败,100 忽略\r\n',
